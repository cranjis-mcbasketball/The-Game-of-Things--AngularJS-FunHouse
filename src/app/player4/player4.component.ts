...
export class Player4Component {
  ...
  drop(event: CdkDragDrop<string[]>) {
    ...
    if ((event.previousContainer.id === "beginning-options") && (event.container.id === "response-submission")) {
      console.log('this.beginnings, event.previousContainer.data', this.beginnings, event.previousContainer.data
      )
    ...
    transferArrayItem(event.previousContainer.data, event.container.data,
    ...
    } else if ((event.previousContainer.id === "middle-options") && (event.container.id === "response-submission")) {
      ...
      if(event.previousContainer.data.length < 6){
        transferArrayItem(event.container.data, event.previousContainer.data,
          1, event.previousContainer.data.length);
      }
    ...
    } else if ((event.previousContainer.id === "end-options") && (event.container.id === "response-submission")) {
      if (event.currentIndex !== 2) {
        event.currentIndex = 2;

      if(event.previousContainer.data.length < 6){
        transferArrayItem(event.container.data, event.previousContainer.data,
          2, event.previousContainer.data.length);
      }
      transferArrayItem(event.previousContainer.data, event.container.data,
